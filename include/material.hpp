...
struct BRDFMaterial : Material {
	...
	BRDFMaterial(std::shared_ptr<Shader> shader, const glm::vec3& diffuse, const glm::vec3& specular, float alpha, int specularity) : shader(shader), diffuse(diffuse), specular(specular), alpha(alpha), specularity(specularity) {}
